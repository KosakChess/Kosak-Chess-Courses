...
export const Volume = () => {
	return (
		<VolumeSlider.Root className="volume-slider group relative mx-[7.5px] inline-flex h-10 w-full max-w-[80px] cursor-pointer touch-none select-none items-center outline-none aria-hidden:hidden">
			<VolumeSlider.Track className="relative z-0 h-[5px] w-full rounded-sm bg-white/30 ring-ring group-data-[focus]:ring-[3px]">
				<VolumeSlider.TrackFill className="absolute h-full w-[var(--slider-fill)] rounded-sm bg-slate-100 will-change-[width]" />
			</VolumeSlider.Track>
			...
			<VolumeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 z-20 size-[15px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity will-change-[left] group-data-[active]:opacity-100 group-data-[dragging]:ring-4" />
		</VolumeSlider.Root>
	);
};
...
export const Time = ({ thumbnails }: TimeSliderProps) => {
	return (
				...
				{(cues, forwardRef) =>
					cues.map((cue) => (
						<div
							...
							ref={forwardRef}
						>
							<TimeSlider.Track className="relative z-0 h-[5px] w-full rounded-sm bg-white/30 ring-ring group-data-[focus]:ring-[3px]">
								<TimeSlider.TrackFill className="absolute h-full w-[var(--chapter-fill)] rounded-sm bg-slate-100 will-change-[width]" />
